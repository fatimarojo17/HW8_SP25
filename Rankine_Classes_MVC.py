...
class rankineView():
    ...
    def selectQualityOrTHigh(self, Model=None):
        ...
        if self.rdo_Quality.isChecked():
            ...
            # Get saturated temperature at PHigh
            pHigh = float(self.le_PHigh.text())
            if not SI:
                pHigh /= UC.bar_to_psi  # Convert psi to bar
            Tsat = Model.steam.getsatProps_p(pHigh).tsat
    ...
    def setNewPHigh(self, Model=None):
        SI = self.rb_SI.isChecked()
        pHigh = float(self.le_PHigh.text())
        pHigh = pHigh if SI else pHigh / UC.bar_to_psi  # Convert psi to bar if needed
        satProps = Model.steam.getsatProps_p(pHigh)
    ...
    def setNewPLow(self, Model=None):
        ...
        pLow = pLow if SI else pLow / UC.bar_to_psi
        satProps = Model.steam.getsatProps_p(pLow)
    ...
    def updateUnits(self, Model=None):
        ...
        # Update pressures shown in the line edits
        ...
        # Update turbine inlet if THigh is selected
        if not self.rdo_Quality.isChecked() and Model.t_high is not None:
            tHigh = Model.t_high
            tHigh = tHigh if Model.SI else UC.C_to_F(tHigh)
            self.le_TurbineInletCondition.setText("{:.2f}".format(tHigh))

        # You can also update label text here if needed
...
class rankineController():
    def __init__(self, *args):
        ...
        if len(args) >= 2:
            ...
            # ✅ Ensure plotting works in GUI:
            self.View.ax = self.DW[-1]
            self.View.canvas = self.DW[-3]
    ...
    def updatePlot(self):
        self.View.plot_cycle_TS(Model=self.Model)
