...
class rankineView():
    ...
    def selectQualityOrTHigh(self, Model=None):
        ...
        if self.rdo_Quality.isChecked():
            ...
            PCF = 1 if Model.SI else UC.psi_to_bar
            p_high = float(self.le_PHigh.text()) * PCF
            satProps = Model.steam.getsatProps_p(p_high)

            # Step 2: convert T to proper units
            T_high = satProps.tsat if Model.SI else UC.C_to_F(satProps.tsat)

            # Step 3: update the line edit
            self.le_TurbineInletCondition.setText(f"{T_high:.2f}")
        ...
        if not self.rdo_Quality.isChecked():
            if not self.rdo_Quality.isChecked():
                ...
                if T_high is not None:
                    T_display = T_high if Model.SI else UC.C_to_F(T_high)
